  ...
  <?php
    ...
    $title = $_POST['title'];
		...
		if($_POST['actiontype']=='insert'){
			...
			if($checkduplicate > 0 ){
				$frmmsg = "This lord story already exist.";
			} else {*/
				$insert = mysql_query("INSERT into story set lord_id='".$lord_id."',title= '".$title."', text='".$storytext."', created_on='".DATETIME."', created_by='".$_SESSION['user_id']."' ");
				if($insert==1){
					//header("location: lord_list.php?add=done");
				echo '<script>window.location.href="lord_story_list.php?add=done"</script>';
				}
			//}
		}
		if($_POST['actiontype']=='update'){
			/*$checkduplicate = checkduplicate('story', 'lord_id', $lord_id, 'id', $_REQUEST['id']);
			
			if($checkduplicate > 0){
				$frmmsg = "This lord story already exist.";
			} else {*/
				$update = mysql_query("update story set lord_id='".$lord_id."', title= '".$title."', text='".$storytext."', updated_on='".DATETIME."', updated_by='".$_SESSION['user_id']."', status='".$status."' where id='".$_REQUEST['id']."'  ");
				if($update){
					$frmmsg = UPDATEMSG;
				}
			//}
	...
	if($_REQUEST['id']!=''){
		$result_row = single_row('story', 'id', $_REQUEST['id']);
	}
                  ...
                  <div class="x_content">
                    ...
                    <form name="addfrm" id="" data-parsley-validate class="form-horizontal form-label-left" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <select name="lordid" required="required" class="form-control col-md-7 col-xs-12">
                            ...
                            <?php
							  ...
							  ?>
							  <option value="<?php echo $row['id']?>" <?php if($result_row['lord_id']==$row['id']) { echo 'selected="selected"';}?>><?php echo $row['name']?></option>
							  <?php } ?>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                         <!--  <textarea  name="storytext" required="required" class="form-control col-md-7 col-xs-12"><?php echo $result_row['text']?></textarea> -->
                         <input type="text" name="title" class="form-control col-md-7 col-xs-12" value="<?php echo $result_row['title'] ?>" required="required">
                        </div>
                      </div>


                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Story <span class="required">*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <textarea  name="storytext" required="required" class="form-control col-md-7 col-xs-12"><?php echo $result_row['text']?></textarea>
                        </div>
