...
<html lang="en">
  <?php
	...
	if($_REQUEST['delete_id']!='' && $_REQUEST['delete_id'] > 0){
		$resultrow = single_row("lords", "id", $_REQUEST['delete_id']);
		@unlink("lord_images/".$resultrow['image']);
		$delete = deleterecord("lords", "id", $_REQUEST['delete_id']);
		if($delete==true){
			$message = DELETEMSG;
		}
	}
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
				  ...
				  <form name="searach" method="get">
					...
					<tr>
						<td>Name</td>
						<td><input type="text" name="lordname" value="<?php echo $_REQUEST['lordname']?>" /></td>
						<td>Status</td>
						...
						<input type="button" name="actionreset" value="Reset" onclick="document.location.href='lord_list.php'" /></td>
					...
					if($_REQUEST['action']=='Search'){
						...
						if($_REQUEST['lordname'] != ''){
							...
							$searchquery .= " and name like '%$lordname%'";
						}
					}
					
					
					$totalquery = mysql_query("select * from lords where id!='' $searchquery");
					
					$select = mysql_query("select * from lords  where id!='' $searchquery order by id desc limit $start, $end");
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  ?>
                        <tr>
						  ...
						  <td><img src="<?php echo $baseurl.'/lord_images/'.$row['image']?>" height="50px" width="50px" /></td>
						  <td><?php 
						  if($row['status']=='0'){
							echo 'In-active';
						  } elseif($row['status']=='1'){
                          ...
                          <td><a href="<?php echo $baseurl?>/add_lord.php?id=<?php echo $row['id']?>">Edit<a/> | <a href="<?php echo $baseurl?>/lord_list.php?delete_id=<?php echo $row['id']?>" onclick="return confirm('Are you sure want to delete?');">Delete<a/>
                        </tr>
					   <?php  $j++;}  ?>
