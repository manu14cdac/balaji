...
<html lang="en">
  <?php
	...
	if($_REQUEST['delete_id']!='' && $_REQUEST['delete_id'] > 0){
		$delete = deleterecord("miracle", "id", $_REQUEST['delete_id']);
		if($delete==true){
			$message = DELETEMSG;
		}
	}
        ...
        <!-- page content -->
          ...
          <div class="">
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
				  ...
				  <form name="searach" method="get">
					<table width="100%">
					<tr>
						<td>Story Teller Name</td>
						<td>
						<!-- <select name="lordname"  class="form-control">
              ...
              <input type="text" name="story_title" class="form-control" value="<?php echo $_REQUEST['story_title']; ?>">
						...
						<input type="button" name="actionreset" value="Reset" onclick="document.location.href='micarle_list.php'" class="btn btn-success" /></td>
					...
					if($_REQUEST['action']=='Search'){
						...
						if($_REQUEST['story_title'] != ''){
							$story_title = $_REQUEST['story_title'];
							$searchquery .= " and story_title = '".$story_title."' ";
						}
					}
					$totalquery = mysql_query("SELECT * from miracle where id!='' $searchquery");					
					$select = mysql_query("SELECT * from miracle where id!='' $searchquery order by id desc limit $start, $end");
              ...
              <th>Story</th>
					  ...
					  ?>
                        <tr>
                          ...
                          <td><?php echo $row['story_teller'];?> </td>
						  <td><?php echo substr($row['miracle_story'],0,50).'...'?></td>
						  <td><?php 
						  if($row['status']=='0'){
							echo 'In-active';
						  } elseif($row['status']=='1'){
                          ...
                          <td><a href="<?php echo $baseurl?>/addmiracle.php?id=<?php echo $row['id']?>">Edit<a/> | <a href="<?php echo $baseurl?>/miracle_list.php?delete_id=<?php echo $row['id']?>" onclick="return confirm('Are you sure want to delete?');">Delete<a/>
                        </tr>
					   <?php  $j++;}  ?>
					...
					<div class="pagination">	
								<?php
									...
									if($_REQUEST['story_title'] !=''){
										$story_title= $_REQUEST['story_title'];
										$searchq .= "&story_title=$story_title";
									} else {
										$searchq .='';
									}									
									...
									$reload = $baseurl."/miracle_list.php";
