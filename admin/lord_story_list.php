...
<html lang="en">
  <?php
	...
	if($_REQUEST['delete_id']!='' && $_REQUEST['delete_id'] > 0){
		$delete = deleterecord("story", "id", $_REQUEST['delete_id']);
		if($delete==true){
			$message = DELETEMSG;
		}
	}
          ...
          <div class="">
            ...
            <div class="row">
                ...
                <div class="x_panel">
				  ...
				  <form name="searach" method="get">
					...
					<tr>
						...
						<input type="button" name="actionreset" value="Reset" onclick="document.location.href='lord_story_list.php'" class="btn btn-success" /></td>
					...
					if($_REQUEST['action']=='Search'){
						...
						if($_REQUEST['lordname'] != ''){
							...
							$searchquery .= " and lord_id = '".$lordname."' ";
						}
					}
					$totalquery = mysql_query("select * from story where id!='' $searchquery");					
					$select = mysql_query("select * from story where id!='' $searchquery order by id desc limit $start, $end");
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  ?>
                        <tr>
                          <td><?PHP echo $j?></td>
                          <td><?php 
							$lordinfo = single_row('lords', 'id', $row['lord_id']);
							echo $lordinfo['name'];?>
						  </td>
              <td> <?php echo $row['title']; ?> </td>
                          ...
                          <td><a href="<?php echo $baseurl?>/add_lord_story.php?id=<?php echo $row['id']?>">Edit<a/> | <a href="<?php echo $baseurl?>/lord_story_list.php?delete_id=<?php echo $row['id']?>" onclick="return confirm('Are you sure want to delete?');">Delete<a/>
                        </tr>
					   <?php  $j++;}  ?>
