...
<html lang="en">
  <?php
	...
	if($_REQUEST['delete_id']!='' && $_REQUEST['delete_id'] > 0){
		$resultrow = single_row("lords_aarti", "id", $_REQUEST['delete_id']);
		@unlink("lord_aarti/".$resultrow['aarti_audio']);
		$delete = deleterecord("lords_aarti", "id", $_REQUEST['delete_id']);
		if($delete==true){
			$message = DELETEMSG;
		}
	}
                ...
                <div class="x_panel">
				  ...
				  <form name="searach" method="get">
					...
					<tr>
						...
						<input type="button" name="actionreset" value="Reset" onclick="document.location.href='aarti_list.php'" class="btn btn-success" /></td>
					...
					$totalquery = mysql_query("select * from lords_aarti where id!='' $searchquery");
					$select = mysql_query("select * from lords_aarti  where id!='' $searchquery order by id desc limit $start, $end");
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  ?>
                        <tr>
                          <td><?PHP echo $j?></td>
                          <td><?php 
						  $lordinfo = single_row('lords', 'id', $row['lordid']);
							echo $lordinfo['name'];?></td>
						  <td>
						  <a href="<?php echo $baseurl.'/lord_aarti/'.$row['aarti_audio']?>"><?php echo $row['aarti_audio']?></a>
						  </td>
						  <td><?php echo $row['youtube_url']?></td>
						  <td><?php echo $row['aarti_text']?></td>
						  <td><?php 
						  if($row['status']=='0'){
							echo 'In-active';
						  } elseif($row['status']=='1'){
                          ...
                          <td><a href="<?php echo $baseurl?>/add_aarti.php?id=<?php echo $row['id']?>">Edit<a/> | <a href="<?php echo $baseurl?>/aarti_list.php?delete_id=<?php echo $row['id']?>" onclick="return confirm('Are you sure want to delete?');">Delete<a/>
                        </tr>
					   <?php  $j++;}  ?>
