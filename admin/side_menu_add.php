  ...
  <?php
    ...
    $url = $_POST['url'];
		...
		if($_POST['actiontype']=='insert'){
			...
			$checkduplicate = checkduplicate('sidemenu', 'name', $name,'','');
			if($checkduplicate > 0 ){
				$frmmsg = "This name already exist.";
			} else {
				
				$insert = mysql_query("INSERT into sidemenu set name='".$name."', url='".$url."',status='".$status."', created_on='".DATETIME."', created_by='".$_SESSION['user_id']."' ");
				if($insert==1){
					//header("location: lord_list.php?add=done");
					echo '<script>window.location.href="side_menu_list.php?add=done"</script>';
				}
			}	
		}
		if($_POST['actiontype']=='update'){
			$checkduplicate = checkduplicate('sidemenu', 'name', $name, 'id', $_REQUEST['id']);
			if($checkduplicate > 0){
				$frmmsg = "This lord name already exist.";
			} else {
				
				$update = mysql_query("UPDATE sidemenu set name='".$name."', url='".$url."', status='".$status."', updated_on='".DATETIME."', updated_by='".$_SESSION['user_id']."' where id='".$_REQUEST['id']."' ");
				if($update){
					$frmmsg = UPDATEMSG;
				}
			}
	...
	if($_REQUEST['id']!=''){
		$result_row = single_row('sidemenu', 'id', $_REQUEST['id']);
	}
                  ...
                  <div class="x_content">
                    ...
                    <form name="addfrm" id="" data-parsley-validate class="form-horizontal form-label-left" method="post" enctype="multipart/form-data">
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Side Menu Name <span class="required">*</span>
                        </label>
