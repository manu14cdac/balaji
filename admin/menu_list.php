...
<html lang="en">
  <?php
	...
	if($_REQUEST['delete_id']!='' && $_REQUEST['delete_id'] > 0){
		$resultrow = single_row("menu", "id", $_REQUEST['delete_id']);
		
		$delete = deleterecord("menu", "id", $_REQUEST['delete_id']);
		if($delete==true){
			$message = DELETEMSG;
		}
	}
                ...
                <div class="x_panel">
				  ...
				  <form name="searach" method="get">
					...
					<tr>
						...
						<input type="button" name="actionreset" value="Reset" onclick="document.location.href='menu_list.php'" /></td>
					...
					$totalquery = mysql_query("SELECT * from menu where id!='' $searchquery");
					
					$select = mysql_query("SELECT * from menu  where id!='' $searchquery order by id desc limit $start, $end");
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
					  ...
					  ?>
                        <tr>
                          ...
                          <td><a href="<?php echo $baseurl?>/add_menu.php?id=<?php echo $row['id']?>">Edit<a/> | <a href="<?php echo $baseurl?>/menu_list.php?delete_id=<?php echo $row['id']?>" onclick="return confirm('Are you sure want to delete?');">Delete<a/>
                        </tr>
					   <?php  $j++;}  ?>
