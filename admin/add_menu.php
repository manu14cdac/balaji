  ...
  <?php
    ...
    $url = $_POST['url'];
		...
		if($_POST['actiontype']=='insert'){
			...
			$checkduplicate = checkduplicate('menu', 'name', $name,'','');
			if($checkduplicate > 0 ){
				$frmmsg = "This name already exist.";
			} else {
				
				$insert = mysql_query("INSERT into menu set name='".$name."', url='".$url."',status='".$status."', created_on='".DATETIME."', created_by='".$_SESSION['user_id']."' ");
				if($insert==1){
					//header("location: lord_list.php?add=done");
					echo '<script>window.location.href="menu_list.php?add=done"</script>';
				}
			}	
		}
		if($_POST['actiontype']=='update'){
			$checkduplicate = checkduplicate('menu', 'name', $name, 'id', $_REQUEST['id']);
			if($checkduplicate > 0){
				$frmmsg = "This lord name already exist.";
			} else {
				
				$update = mysql_query("UPDATE menu set name='".$name."', url='".$url."', status='".$status."', updated_on='".DATETIME."', updated_by='".$_SESSION['user_id']."' where id='".$_REQUEST['id']."' ");
				if($update){
					$frmmsg = UPDATEMSG;
				}
			}
	...
	if($_REQUEST['id']!=''){
		$result_row = single_row('menu', 'id', $_REQUEST['id']);
	}
